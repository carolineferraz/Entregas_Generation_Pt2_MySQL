...
alter table tb_classes rename column desricao to descricao;
...
insert into tb_personagens (nome,cidade,ataque,defesa,id_classe) values ('thor','argasd',500,5000,1);
...
insert into tb_personagens (nome,cidade,ataque,defesa,id_classe) values ('Mario','Marioworld',3000,2000,2);
insert into tb_personagens (nome,cidade,ataque,defesa,id_classe) values ('Bob Esponja','Fenda do Biquini',1900,4500,5);
insert into tb_personagens (nome,ataque,defesa,id_classe) values ('Merlin',5000,3700,1);
...
#inner join (inner join e left join se comportam parecido)
select * from tb_personagens 
	inner join tb_classes on tb_classes.id = tb_personagens.id_classe;

#inner join buscando colunas específicas:
...
#inner join buscando personagens de uma classe especifica:
select tb_personagens.nome, tb_classes.classe, tb_classes.descricao from tb_personagens inner join 
